...
const Verify = () => {
  ...
  const handleVerify = async () => {
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/api/verify/${certificateId}`);
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-base-200 p-6">
      <h1 className="text-4xl font-bold text-center mb-6 text-primary">Verify Certificate</h1>

      <div className="card bg-white shadow-xl p-6 w-full max-w-lg rounded-lg">
        <label className="block text-lg font-semibold mb-2">Enter Certificate ID:</label>
        <input
          type="text"
  ...
  <div className="mt-6 p-4 bg-amber-500 border-l-4 border-green-500 rounded-lg">
    <h2 className="text-xl font-semibold text-green-700">Certificate Verified ✅</h2>
    ...
    {certificateData.qrCodeUrl && (
      <div className="mt-4 text-center">
        ...
        <a href={certificateData.qrCodeUrl} download="certificate_qr.png">
          <img src={certificateData.qrCodeUrl} alt="QR Code" className="w-32 h-32 mx-auto border p-2 rounded-lg shadow-md cursor-pointer hover:opacity-80" />
        </a>
